...
let warn_bad_docstrings () =
  if Utils.Warnings.is_active (Utils.Warnings.Unexpected_docstring true) then begin
    List.iter
      (fun ds ->
         match ds.ds_attached with
         | Info -> ()
         | Unattached ->
           prerr_warning ds.ds_loc (Utils.Warnings.Unexpected_docstring true)
         | Docs ->
             match ds.ds_associated with
             | Zero | One -> ()
             | Many ->
               prerr_warning ds.ds_loc (Utils.Warnings.Unexpected_docstring false))
      (List.rev !docstrings)
end
